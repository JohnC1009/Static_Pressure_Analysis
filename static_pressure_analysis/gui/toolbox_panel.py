...
# Equipment entries: (NodeType, icon_char, color)
EQUIPMENT = [
    (NodeType.FAN,              "F",  "#4A90D9"),
    (NodeType.DAMPER,           "D",  "#E8943A"),
    (NodeType.MIXING_PLENUM,    "M",  "#6BBF6B"),
    (NodeType.DUCT_SPLIT,       "Y",  "#C47DD8"),
    (NodeType.PRESSURE_OUTPUT,  "P",  "#E05555"),
]
...
class EquipmentDragButton(QPushButton):
    ...
    def __init__(self, node_type: NodeType, icon_char: str,
                 color: str, parent=None):
        super().__init__(parent)
        self.node_type = node_type
        self.setText(f"  {node_type.value}")
    ...
    def mouseMoveEvent(self, event):
        if (self._drag_start is not None
            ...
            drag = QDrag(self)
            mime = QMimeData()
            mime.setText(self.node_type.value)
...
class ToolboxPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for node_type, icon_char, color in EQUIPMENT:
            btn = EquipmentDragButton(node_type, icon_char, color)
