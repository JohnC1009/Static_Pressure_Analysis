"""Visual connector (ductwork line) between two ports on the canvas."""
...
class TempConnectorItem(QGraphicsPathItem):
    """Temporary connector shown while the user is dragging a new connection."""
    ...
    def update_end(self, end_pos: QPointF):
        dx = abs(end_pos.x() - self._start.x()) * 0.5
        dx = max(dx, 40)
